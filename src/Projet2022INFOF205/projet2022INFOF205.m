...
for step = 1:nsteps
   ...
   vxhatE(step)= sqrt(vxhat2);
   
   %%% xhat(t+h)= xhat(t) + h * vxhat 
   %%%          = xhat(t) + h * sign(vxhat)*vxhat %%% enonce
   %xhatE(step+1)= xhatE(step) + h * vxhat;
   %xhatE(step+1)= xhatE(step) + h*sign(vxhat)*vxhatE(step);
   xhatE(step+1)= xhatE(step) + yhatE(step)*vxhatE(step);
   ...
   if vxhatE(step)==0   %%%
      ...
      %%% axhat = à compléter ... vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
      axhat = g*(abs(slopehatE(step))/(1+(slopehatE(step)).^2));    %%% enonce
      %axhat
      %%% completion fin ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
if playvideo==true, figure(1)   %%%
   ...
   %wheelcolor
   %            tangency,
   %rollonhills(x, y,    v,    r,   color,     frames quantity)
   %rollonhills(xx, yy,  vhatE,r,   wheelcolor)    
   nsteps
   rollonhills(xx, yy,   vhatE,0.03,wheelcolor) %%%
   
   %rollonhillsenergiesviz(nsteps, xhatE, vxhatE, Epot, Ecin, Etot, xx,yy,vhatE,0.03,wheelcolor, 4000)
